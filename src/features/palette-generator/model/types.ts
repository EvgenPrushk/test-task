...
> = {
  [K in keyof TInput as `${K & string}_${TTone["_name"] &
    string}`]?: ReturnType<TTone>;
};
  ...
  ? UnionToIntersection<
      {
        [SubtoneKey in keyof TTone["_subtones"]]: {
          [K in keyof TInput as `${K & string}_${SubtoneKey &
            string}_${TTone["_name"] & string}`]?: ReturnType<
            TTone["_subtones"][SubtoneKey]
          >;
        };
...
> = BaseColorOutput<TInput, TConfig["base"]> &
  (TConfig["tones"] extends Record<
    string,
    Tone<TransformCallback, { name?: string; subtone?: SubtoneMap }>
  >
    ? UnionToIntersection<
        {
          [ToneKey in keyof TConfig["tones"]]: ToneColorOutput<
            TInput,
            TConfig["tones"][ToneKey]
          > &
