...
1. Убедитесь, что у вас есть доступ к репозиторию. Если нет, обратитесь к администратору  по telegram @angelmaximalist.
...
3. Запустите :
   ...
   npm run start
   ```
4. Запустить тестa (помогло выявить 3 ошибки)  с тестами можно поиграться в файле (index.test.tsx) :

   ```
   npx jest
   ```
5. С данными можно и функциями можно по экспериментировать в файле  colors.ts и сразу помотреть визуально.

6. Output palette можно глянуть в консоли разработчика F12 ('Output palette')
